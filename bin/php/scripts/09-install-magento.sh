...
if docker_exec_php "[ -f '${MAGENTO_ROOT}/bin/magento' ]"; then
    ...
    docker_exec_php_root "find '${MAGENTO_ROOT}' -mindepth 1 -maxdepth 1 -exec rm -rf {} +"
...
if docker_exec_php "[ -f '${MAGENTO_ROOT}/bin/magento' ]" && docker_exec_php "cd '${MAGENTO_ROOT}' && php bin/magento setup:config:status >/dev/null 2>&1"; then
