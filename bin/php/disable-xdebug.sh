...
info "Disabling Xdebug in ${XDEBUG_INI}"
...
replace_ini_setting "xdebug.mode" "off"
replace_ini_setting "xdebug.start_with_request" "no"
replace_ini_setting "xdebug.discover_client_host" "false"

if grep -qE "^[[:space:]]*zend_extension=.*xdebug" "$XDEBUG_INI"; then
    info "Xdebug extension remains loaded; module will be disabled by mode=off"
else
    info "xdebug extension line not found; verify PHP image if no extension is available"
fi
...
ok "Xdebug disabled and php-fpm rebuilt/restarted"
